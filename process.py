...
def read_latest_results():
    ...
    if not os.path.exists(latest_file):
        ...
        try:
            ...
            print('Could not open / read from file :' + latest_file + ', error:' + err)
            iff.close()
            return latest_results
    latest_results = json.loads(text_contents)
    ...
    for idx in range(0, len(results)):
        ...
        for idx2 in range(idx + 1, len(results)):
            ...
            if results[idx2]["numbers"] == results[idx]["numbers"]:
                print("\n%s %s %s / %s: %s / %s %s %s\n" % (
                    results[idx]["year"], results[idx]["week"],
                    results[idx]["numbers"], results[idx]["adds"],
                    results[idx2]["numbers"], results[idx2]["adds"],
                    results[idx2]["year"], results[idx2]["week"]))
                current_match = 7
            else:
                matches = compare(
                    results[idx]["numbers"], results[idx]["adds"],
...
def read_from_file(filename):
    ...
    if not os.path.exists(filename):
        ...
        try:
            ...
            print('Could not open / read from file :' + filename + ', error:' + err)
            iff.close()
            ...
            for draw in j["draws"]:
                ...
                for prize in draw["prizeTiers"]:
                    if win_line == prize["name"]:
                        return "%8d.%02d" % (prize["shareAmount"] / 100, prize["shareAmount"] % 100)
        except:
...
def skip_repetition(mains, adds):
    ...
    for idx in range(0, len(results)):
        matches = compare(
            results[idx]["numbers"], results[idx]["adds"],
            mains, adds)
    if matches[0] > 3 or (matches[0] == 3 and matches[1] > 0) or (matches[0] == 2 and matches[1] == 2):
        return True
...
def gen_stat(idx):
    ...
    if stat_debug:
        print("idx %d, stat_adds_sorted: %s, stats_adds: %s" %
    ...
    for useless_var in range(0, settings["iterations"]):
        ...
        for idx in range(0, len(results)-1):
            for rounds_idx in range(0, 1):
                ...
                (nums, adds) = compare(num1, num2, results[idx+1]["numbers"],
                                       results[idx+1]["adds"])
                if nums >= 2:
                    ...
                    if not settings["quiet"]:
                        print("Win %d-%d %s/%2s : %20s %7s / %20s %7s %s" %
                              (nums, adds, w_year, w_week,
                               num1, num2, results[idx+1]["numbers"], results[idx+1]["adds"],
                               w_money))
                    wins += 1
        print("Rate: %3d / %d (%3.4f), - / +: %d / %7.2f, skipped %d" %
